...
// $db = new MysqliDb (getenv('DB_HOST'), 'root', 'SU-mon5', 'su');

$input = file_get_contents('php://input');
$input = json_decode($input, TRUE); //convert JSON into array
...
function validateLocation($location_name, $locations) {
    foreach($locations as $location) {
        if($location['location_name'] == $feedback['location']) {
            return $location['id'];
        }
    }
...
function getLocations() {
    ...
    $result = $db->get('location', null, ['location_name', 'location_id']);
...
function insertRow($feedback, $feedback_location, $input, $locations) {
    $db = new MysqliDb ('mysql_host', 'web', 'viv3nij', 'su');
    if(is_null($locations)) {
        return NULL;
    } else {
        foreach($locations as $location) {
            if($location['location_name'] == $feedback_location) {
                $data = Array (
                    "name" => $input['name'],
                    "email" => $input['email'],
                    "text" => $feedback,
                    "location" => (int) $location['location_id']
                );
                // var_dump($location['location_id']);
                $id = $db->insert('feedback', $data);
                // var_dump($id);
                // echo $db->getLastErrno();
                if(!$id) {
                    return NULL;
                } else {
                    return $id;
...
// Main workflow
if(!validateInput($GLOBALS['input'])) {
    header('HTTP', true, 400);
}
else {
    ...
    foreach ($input['feedback'] as $location => $feedback) {
        $id = insertRow($feedback, $location, $input, $locations);
    ...
    if($flag) {
        ...
        $message .= $before_name_email;
        
        if($input['name'] != '') {
            $message .= '<p>Name: ' . $input['name'] . '</p>';
        }

        $email = '';
        if($input['email'] != '') {
            $message .= '<p>Email: ' . $input['email'] . '</p>';
            $email = $input['email'];
        }
        

        $message .= $after_name_email;

        foreach ($input['feedback'] as $location => $feedback) {
            $loc_feedback = $feedback_template;
            $loc_feedback = str_replace('%location_name%', $location, $loc_feedback);
            $loc_feedback = str_replace('%location_feedback%', $feedback, $loc_feedback);

            $message .= $loc_feedback;
        }
        ...
        $mail_success = mail('su-web@email.arizona.edu', $subject, $message, $headers);
