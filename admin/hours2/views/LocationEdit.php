...
loadjs.ready("head", function () {
    ...
    flocationedit = currentForm = new ew.Form("flocationedit", "edit");

    // Add fields
    var fields = ew.vars.tables.location.fields;
    flocationedit.addFields([
    ...
    // Validate form
    flocationedit.validate = function () {
    ...
    // Form_CustomValidate
    flocationedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    flocationedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    flocationedit.lists.group_hours = <?= $Page->group_hours->toClientList($Page) ?>;
    flocationedit.lists.breakfast = <?= $Page->breakfast->toClientList($Page) ?>;
    flocationedit.lists.lunch = <?= $Page->lunch->toClientList($Page) ?>;
    flocationedit.lists.dinner = <?= $Page->dinner->toClientList($Page) ?>;
    flocationedit.lists.continuous = <?= $Page->continuous->toClientList($Page) ?>;
    loadjs.done("flocationedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="flocationedit" id="flocationedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="hidden" data-table="location" data-field="x_location_id" data-hidden="1" name="o_location_id" id="o_location_id" value="<?= HtmlEncode($Page->location_id->OldValue ?? $Page->location_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
