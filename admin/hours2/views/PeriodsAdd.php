...
?>
<script>
if (!ew.vars.tables.periods) ew.vars.tables.periods = <?= JsonEncode(GetClientVar("tables", "periods")) ?>;
...
loadjs.ready("head", function () {
    ...
    fperiodsadd = currentForm = new ew.Form("fperiodsadd", "add");

    // Add fields
    var fields = ew.vars.tables.periods.fields;
    ...
    fperiodsadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
$Page->showMessage();
?>
<form name="fperiodsadd" id="fperiodsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="<?= $Page->start_date->getInputTextType() ?>" data-table="periods" data-field="x_start_date" name="x_start_date" id="x_start_date" placeholder="<?= HtmlEncode($Page->start_date->getPlaceHolder()) ?>" value="<?= $Page->start_date->EditValue ?>"<?= $Page->start_date->editAttributes() ?> aria-describedby="x_start_date_help">
...
loadjs.ready(["fperiodsadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fperiodsadd", "x_start_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<input type="<?= $Page->end_date->getInputTextType() ?>" data-table="periods" data-field="x_end_date" name="x_end_date" id="x_end_date" placeholder="<?= HtmlEncode($Page->end_date->getPlaceHolder()) ?>" value="<?= $Page->end_date->EditValue ?>"<?= $Page->end_date->editAttributes() ?> aria-describedby="x_end_date_help">
...
<script>
loadjs.ready(["fperiodsadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fperiodsadd", "x_end_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->type->isInvalidClass() ?>" data-table="periods" data-field="x_type" name="x_type[]" id="x_type_299263" value="1"<?= ConvertToBool($Page->type->CurrentValue) ? " checked" : "" ?><?= $Page->type->editAttributes() ?> aria-describedby="x_type_help">
    <label class="custom-control-label" for="x_type_299263"></label>
</div>
