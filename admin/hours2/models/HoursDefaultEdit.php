...
class HoursDefaultEdit extends HoursDefault
{
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // mon_open
            $this->mon_open->setDbValueDef($rsnew, $this->mon_open->CurrentValue, null, $this->mon_open->ReadOnly);

            // mon_close
            $this->mon_close->setDbValueDef($rsnew, $this->mon_close->CurrentValue, null, $this->mon_close->ReadOnly);

            // tue_open
            $this->tue_open->setDbValueDef($rsnew, $this->tue_open->CurrentValue, null, $this->tue_open->ReadOnly);

            // tue_close
            $this->tue_close->setDbValueDef($rsnew, $this->tue_close->CurrentValue, null, $this->tue_close->ReadOnly);

            // wed_open
            $this->wed_open->setDbValueDef($rsnew, $this->wed_open->CurrentValue, null, $this->wed_open->ReadOnly);

            // wed_close
            $this->wed_close->setDbValueDef($rsnew, $this->wed_close->CurrentValue, null, $this->wed_close->ReadOnly);

            // thu_open
            $this->thu_open->setDbValueDef($rsnew, $this->thu_open->CurrentValue, null, $this->thu_open->ReadOnly);

            // thu_close
            $this->thu_close->setDbValueDef($rsnew, $this->thu_close->CurrentValue, null, $this->thu_close->ReadOnly);

            // fri_open
            $this->fri_open->setDbValueDef($rsnew, $this->fri_open->CurrentValue, null, $this->fri_open->ReadOnly);

            // fri_close
            $this->fri_close->setDbValueDef($rsnew, $this->fri_close->CurrentValue, null, $this->fri_close->ReadOnly);

            // sat_open
            $this->sat_open->setDbValueDef($rsnew, $this->sat_open->CurrentValue, null, $this->sat_open->ReadOnly);

            // sat_close
            $this->sat_close->setDbValueDef($rsnew, $this->sat_close->CurrentValue, null, $this->sat_close->ReadOnly);

            // sun_open
            $this->sun_open->setDbValueDef($rsnew, $this->sun_open->CurrentValue, null, $this->sun_open->ReadOnly);

            // sun_close
            $this->sun_close->setDbValueDef($rsnew, $this->sun_close->CurrentValue, null, $this->sun_close->ReadOnly);
