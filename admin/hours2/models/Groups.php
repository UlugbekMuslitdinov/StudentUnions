...
class Groups extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // group_id
        $this->group_id = new DbField('groups', 'groups', 'x_group_id', 'group_id', '`group_id`', '`group_id`', 3, 11, -1, false, '`group_id`', false, false, false, 'FORMATTED TEXT', 'NO');
        $this->group_id->IsAutoIncrement = true; // Autoincrement field
        ...
        // group_name
        $this->group_name = new DbField('groups', 'groups', 'x_group_name', 'group_name', '`group_name`', '`group_name`', 200, 255, -1, false, '`group_name`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->group_name->Sortable = true; // Allow sort
        $this->Fields['group_name'] = &$this->group_name;

        // active
        $this->active = new DbField('groups', 'groups', 'x_active', 'active', '`active`', '`active`', 200, 8, -1, false, '`active`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // group_key
        $this->group_key = new DbField('groups', 'groups', 'x_group_key', 'group_key', '`group_key`', '`group_key`', 200, 64, -1, false, '`group_key`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->group_key->Nullable = false; // NOT NULL field
        $this->group_key->Required = true; // Required field
        $this->group_key->Sortable = true; // Allow sort
        $this->Fields['group_key'] = &$this->group_key;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`groups`";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->group_id->setDbValue($conn->lastInsertId());
            $rs['group_id'] = $this->group_id->DbValue;
        }
        return $success;
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->group_key->DbValue = $row['group_key'];
    }
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->group_id->CurrentValue = $keys[0];
            } else {
                $this->group_id->OldValue = $keys[0];
            }
        }
    }
    ...
    {
        global $Language;
        if ($pageName == "GroupsView") {
            return $Language->phrase("View");
        } elseif ($pageName == "GroupsEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "GroupsAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("GroupsView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("GroupsView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "GroupsAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "GroupsAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("GroupsEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("GroupsAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("GroupsDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->group_id->CurrentValue = $key;
            } else {
                $this->group_id->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        // group_key
        $this->group_key->EditAttrs["class"] = "form-control";
        $this->group_key->EditCustomAttributes = "";
        if (!$this->group_key->Raw) {
            $this->group_key->CurrentValue = HtmlDecode($this->group_key->CurrentValue);
        }
        $this->group_key->EditValue = $this->group_key->CurrentValue;
        $this->group_key->PlaceHolder = RemoveHtml($this->group_key->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->group_key);
                } else {
                    $doc->exportCaption($this->group_id);
                    $doc->exportCaption($this->group_name);
                    $doc->exportCaption($this->active);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->group_key);
                    } else {
                        $doc->exportField($this->group_id);
                        $doc->exportField($this->group_name);
                        $doc->exportField($this->active);
