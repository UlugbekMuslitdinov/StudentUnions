 ...
 * Page class
 */
class GroupsAdd extends Groups
{
    use MessagesTrait;
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("GroupsList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "GroupsList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "GroupsView") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'group_key' first before field var 'x_group_key'
        $val = $CurrentForm->hasValue("group_key") ? $CurrentForm->getValue("group_key") : $CurrentForm->getValue("x_group_key");
        if (!$this->group_key->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->group_key->Visible = false; // Disable update for API request
            } else {
                $this->group_key->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->group_key->CurrentValue = $this->group_key->FormValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row['group_key'] = $this->group_key->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->group_key->EditValue = HtmlEncode($this->group_key->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->group_key->Required) {
            if (!$this->group_key->IsDetailKey && EmptyValue($this->group_key->FormValue)) {
                $this->group_key->addErrorMessage(str_replace("%s", $this->group_key->caption(), $this->group_key->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // group_key
        $this->group_key->setDbValueDef($rsnew, $this->group_key->CurrentValue, "", strval($this->group_key->CurrentValue) == "");
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("GroupsList"), "", $this->TableVar, true);
