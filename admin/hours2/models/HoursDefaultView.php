 ...
 * Page class
 */
class HoursDefaultView extends HoursDefault
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "HoursDefaultView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("hour_id") ?? Route("hour_id")) !== null) {
                ...
                $this->RecKey["hour_id"] = $this->hour_id->QueryStringValue;
            } elseif (Post("hour_id") !== null) {
                $this->hour_id->setFormValue(Post("hour_id"));
                $this->RecKey["hour_id"] = $this->hour_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->hour_id->setQueryStringValue($keyValue);
                $this->RecKey["hour_id"] = $this->hour_id->QueryStringValue;
            } else {
                $returnUrl = "HoursDefaultList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "HoursDefaultList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "HoursDefaultList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
