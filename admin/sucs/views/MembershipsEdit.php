...
// Page object
$MembershipsEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fmembershipsedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fmembershipsedit = currentForm = new ew.Form("fmembershipsedit", "edit");
    ...
    fmembershipsedit.addFields([
        ["membership_id", [fields.membership_id.visible && fields.membership_id.required ? ew.Validators.required(fields.membership_id.caption) : null], fields.membership_id.isInvalid],
    ...
    // Validate form
    fmembershipsedit.validate = function () {
    ...
    // Form_CustomValidate
    fmembershipsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmembershipsedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fmembershipsedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fmembershipsedit" id="fmembershipsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->membership_id->Visible) { // membership_id ?>
    <div id="r_membership_id" class="form-group row">
        <label id="elh_memberships_membership_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->membership_id->caption() ?><?= $Page->membership_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->membership_id->cellAttributes() ?>>
<span id="el_memberships_membership_id">
<span<?= $Page->membership_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->membership_id->getDisplayValue($Page->membership_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="memberships" data-field="x_membership_id" data-hidden="1" name="x_membership_id" id="x_membership_id" value="<?= HtmlEncode($Page->membership_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
