 ...
 * Page class
 */
class MembershipsEdit extends Memberships
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MembershipsEdit";
    ...
    public function __construct()
    {
        ...
        // Table object (memberships)
        if (!isset($GLOBALS["memberships"]) || get_class($GLOBALS["memberships"]) == PROJECT_NAMESPACE . "memberships") {
            $GLOBALS["memberships"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'memberships');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("memberships"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->membership_id->setVisibility();
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("membership_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->membership_id->setQueryStringValue($keyValue);
                $this->membership_id->setOldValue($this->membership_id->QueryStringValue);
            } elseif (Post("membership_id") !== null) {
                $this->membership_id->setFormValue(Post("membership_id"));
                $this->membership_id->setOldValue($this->membership_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("membership_id") ?? Route("membership_id")) !== null) {
                    $this->membership_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->membership_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("MembershipsList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "MembershipsList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'membership_id' first before field var 'x_membership_id'
        $val = $CurrentForm->hasValue("membership_id") ? $CurrentForm->getValue("membership_id") : $CurrentForm->getValue("x_membership_id");
        if (!$this->membership_id->IsDetailKey) {
            $this->membership_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->membership_id->CurrentValue = $this->membership_id->FormValue;
    ...
    protected function validateForm()
    {
        ...
        if ($this->membership_id->Required) {
            if (!$this->membership_id->IsDetailKey && EmptyValue($this->membership_id->FormValue)) {
                $this->membership_id->addErrorMessage(str_replace("%s", $this->membership_id->caption(), $this->membership_id->RequiredErrorMessage));
            }
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("MembershipsList"), "", $this->TableVar, true);
        $pageId = "edit";
