 ...
 * Page class
 */
class ExchDepartmentsEdit extends ExchDepartments
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ExchDepartmentsEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("department_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->department_id->setQueryStringValue($keyValue);
                $this->department_id->setOldValue($this->department_id->QueryStringValue);
            } elseif (Post("department_id") !== null) {
                $this->department_id->setFormValue(Post("department_id"));
                $this->department_id->setOldValue($this->department_id->FormValue);
            } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("ExchDepartmentsList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "ExchDepartmentsList") {
