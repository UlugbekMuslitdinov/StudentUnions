...
// Page object
$BursarPaymentEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { bursar_payment: currentTable } });
var currentForm, currentPageID;
var fbursar_paymentedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fbursar_paymentedit = new ew.Form("fbursar_paymentedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fbursar_paymentedit;

    // Add fields
    var fields = currentTable.fields;
    fbursar_paymentedit.addFields([
        ["bursar_id", [fields.bursar_id.visible && fields.bursar_id.required ? ew.Validators.required(fields.bursar_id.caption) : null], fields.bursar_id.isInvalid],
    ...
    // Form_CustomValidate
    fbursar_paymentedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fbursar_paymentedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("fbursar_paymentedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fbursar_paymentedit" id="fbursar_paymentedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->bursar_id->Visible) { // bursar_id ?>
    <div id="r_bursar_id"<?= $Page->bursar_id->rowAttributes() ?>>
        <label id="elh_bursar_payment_bursar_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->bursar_id->caption() ?><?= $Page->bursar_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->bursar_id->cellAttributes() ?>>
<span id="el_bursar_payment_bursar_id">
<span<?= $Page->bursar_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->bursar_id->getDisplayValue($Page->bursar_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="bursar_payment" data-field="x_bursar_id" data-hidden="1" name="x_bursar_id" id="x_bursar_id" value="<?= HtmlEncode($Page->bursar_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fbursar_paymentedit", "datetimepicker"], function () {
    let options = {
        localization: {
    ...
    ew.createDateTimePicker("fbursar_paymentedit", "x_transaction_time", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
