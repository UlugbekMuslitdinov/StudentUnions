 ...
 * Page class
 */
class CardInfoEdit extends CardInfo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CardInfoEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("card_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->card_id->setQueryStringValue($keyValue);
                $this->card_id->setOldValue($this->card_id->QueryStringValue);
            } elseif (Post("card_id") !== null) {
                $this->card_id->setFormValue(Post("card_id"));
                $this->card_id->setOldValue($this->card_id->FormValue);
            } else {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // guest_id
            $this->guest_id->setDbValueDef($rsnew, $this->guest_id->CurrentValue, null, $this->guest_id->ReadOnly);
