 ...
 * Page class
 */
class ChargePaymentView extends ChargePayment
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ChargePaymentView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("charge_id") ?? Route("charge_id")) !== null) {
            $this->RecKey["charge_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("charge_id") ?? Route("charge_id")) !== null) {
                ...
                $this->RecKey["charge_id"] = $this->charge_id->QueryStringValue;
            } elseif (Post("charge_id") !== null) {
                $this->charge_id->setFormValue(Post("charge_id"));
                $this->RecKey["charge_id"] = $this->charge_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->charge_id->setQueryStringValue($keyValue);
                $this->RecKey["charge_id"] = $this->charge_id->QueryStringValue;
            } else {
                $returnUrl = "ChargePaymentList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "ChargePaymentList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "ChargePaymentList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
