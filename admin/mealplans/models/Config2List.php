 ...
 * Page class
 */
class Config2List extends Config2
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "Config2List";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "Config2Add";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "Config2Delete";
        $this->MultiUpdateUrl = "Config2Update";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->min_deposit); // min_deposit
            $this->updateSort($this->max_deposit); // max_deposit
            $this->updateSort($this->current_term_code); // current_term_code
            $this->updateSort($this->fall_term_code); // fall_term_code
            $this->updateSort($this->spring_term_code); // spring_term_code
            $this->updateSort($this->full_year_begin); // full_year_begin
            $this->updateSort($this->half_year_begin); // half_year_begin
            $this->updateSort($this->year_end); // year_end
            $this->updateSort($this->plus_signup_full); // plus_signup_full
            $this->updateSort($this->plus_signup_half); // plus_signup_half
            $this->updateSort($this->bursar_deposit_deadline); // bursar_deposit_deadline
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->setSessionOrderBy($orderBy);
                $this->id->setSort("");
                $this->min_deposit->setSort("");
                $this->max_deposit->setSort("");
                $this->current_term_code->setSort("");
                $this->fall_term_code->setSort("");
                $this->spring_term_code->setSort("");
                $this->full_year_begin->setSort("");
                $this->half_year_begin->setSort("");
                $this->year_end->setSort("");
                $this->plus_signup_full->setSort("");
                $this->plus_signup_half->setSort("");
                $this->bursar_deposit_deadline->setSort("");
            }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fconfig2list\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fconfig2list\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("min_deposit", $this->createColumnOption("min_deposit"));
            $option->add("max_deposit", $this->createColumnOption("max_deposit"));
            $option->add("current_term_code", $this->createColumnOption("current_term_code"));
            $option->add("fall_term_code", $this->createColumnOption("fall_term_code"));
            $option->add("spring_term_code", $this->createColumnOption("spring_term_code"));
            $option->add("full_year_begin", $this->createColumnOption("full_year_begin"));
            $option->add("half_year_begin", $this->createColumnOption("half_year_begin"));
            $option->add("year_end", $this->createColumnOption("year_end"));
            $option->add("plus_signup_full", $this->createColumnOption("plus_signup_full"));
            $option->add("plus_signup_half", $this->createColumnOption("plus_signup_half"));
            $option->add("bursar_deposit_deadline", $this->createColumnOption("bursar_deposit_deadline"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fconfig2srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fconfig2srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fconfig2list"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function newRow()
    {
        $row = [];
        $row['id'] = null;
        $row['min_deposit'] = null;
        $row['max_deposit'] = null;
        $row['current_term_code'] = null;
        $row['fall_term_code'] = null;
        $row['spring_term_code'] = null;
        $row['full_year_begin'] = null;
        $row['half_year_begin'] = null;
        $row['year_end'] = null;
        $row['plus_signup_full'] = null;
        $row['plus_signup_half'] = null;
        $row['bursar_deposit_deadline'] = null;
        return $row;
    }
