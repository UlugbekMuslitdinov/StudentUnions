 ...
 * Page class
 */
class CardInfoView extends CardInfo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CardInfoView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("card_id") ?? Route("card_id")) !== null) {
                ...
                $this->RecKey["card_id"] = $this->card_id->QueryStringValue;
            } elseif (Post("card_id") !== null) {
                $this->card_id->setFormValue(Post("card_id"));
                $this->RecKey["card_id"] = $this->card_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->card_id->setQueryStringValue($keyValue);
                $this->RecKey["card_id"] = $this->card_id->QueryStringValue;
            } else {
                $returnUrl = "CardInfoList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "CardInfoList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "CardInfoList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
