 ...
 * Page class
 */
class CardInfoList extends CardInfo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CardInfoList";
    ...
    public $FormName = "fcard_infolist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "CardInfoAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "CardInfoDelete";
        $this->MultiUpdateUrl = "CardInfoUpdate";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->cust_id->AdvancedSearch->toJson(), ","); // Field cust_id
        $filterList = Concat($filterList, $this->guest_id->AdvancedSearch->toJson(), ","); // Field guest_id
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fcard_infosrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field cust_id
        $this->cust_id->AdvancedSearch->SearchValue = @$filter["x_cust_id"];
        $this->cust_id->AdvancedSearch->SearchOperator = @$filter["z_cust_id"];
        $this->cust_id->AdvancedSearch->SearchCondition = @$filter["v_cust_id"];
        $this->cust_id->AdvancedSearch->SearchValue2 = @$filter["y_cust_id"];
        $this->cust_id->AdvancedSearch->SearchOperator2 = @$filter["w_cust_id"];
        $this->cust_id->AdvancedSearch->save();

        // Field guest_id
        $this->guest_id->AdvancedSearch->SearchValue = @$filter["x_guest_id"];
        $this->guest_id->AdvancedSearch->SearchOperator = @$filter["z_guest_id"];
        $this->guest_id->AdvancedSearch->SearchCondition = @$filter["v_guest_id"];
        $this->guest_id->AdvancedSearch->SearchValue2 = @$filter["y_guest_id"];
        $this->guest_id->AdvancedSearch->SearchOperator2 = @$filter["w_guest_id"];
        $this->guest_id->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->card_id); // card_id
            $this->updateSort($this->cust_id); // cust_id
            $this->updateSort($this->guest_id); // guest_id
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->cust_id->setSort("");
                $this->guest_id->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcard_infolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcard_infolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->card_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("card_id", $this->createColumnOption("card_id"));
            $option->add("cust_id", $this->createColumnOption("cust_id"));
            $option->add("guest_id", $this->createColumnOption("guest_id"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcard_infosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcard_infosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcard_infolist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-bs-toggle=\"button\" data-form=\"fcard_infosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
