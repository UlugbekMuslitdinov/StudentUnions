 ...
 * Page class
 */
class BursarPaymentView extends BursarPayment
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "BursarPaymentView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("bursar_id") ?? Route("bursar_id")) !== null) {
            $this->RecKey["bursar_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("bursar_id") ?? Route("bursar_id")) !== null) {
                ...
                $this->RecKey["bursar_id"] = $this->bursar_id->QueryStringValue;
            } elseif (Post("bursar_id") !== null) {
                $this->bursar_id->setFormValue(Post("bursar_id"));
                $this->RecKey["bursar_id"] = $this->bursar_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->bursar_id->setQueryStringValue($keyValue);
                $this->RecKey["bursar_id"] = $this->bursar_id->QueryStringValue;
            } else {
                $returnUrl = "BursarPaymentList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "BursarPaymentList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "BursarPaymentList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
