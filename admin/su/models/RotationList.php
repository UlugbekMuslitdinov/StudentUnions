 ...
 * Page class
 */
class RotationList extends Rotation
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'rotation';

    // Page object name
    public $PageObjName = "RotationList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "frotationlist";
    ...
    public function __construct()
    {
        ...
        // Table object (rotation)
        if (!isset($GLOBALS["rotation"]) || get_class($GLOBALS["rotation"]) == PROJECT_NAMESPACE . "rotation") {
        ...
        $this->AddUrl = "RotationAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "RotationDelete";
        $this->MultiUpdateUrl = "RotationUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'rotation');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option frotationlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("rotation"));
                $doc->Text = @$content;
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->file_name->AdvancedSearch->toJson(), ","); // Field file_name
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "frotationlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field file_name
        $this->file_name->AdvancedSearch->SearchValue = @$filter["x_file_name"];
        $this->file_name->AdvancedSearch->SearchOperator = @$filter["z_file_name"];
        $this->file_name->AdvancedSearch->SearchCondition = @$filter["v_file_name"];
        $this->file_name->AdvancedSearch->SearchValue2 = @$filter["y_file_name"];
        $this->file_name->AdvancedSearch->SearchOperator2 = @$filter["w_file_name"];
        $this->file_name->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->file_name, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->file_name); // file_name
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"frotationlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"frotationlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.frotationlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"frotationlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
