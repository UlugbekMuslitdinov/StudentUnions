 ...
 * Page class
 */
class RestaurantsSlidesAdd extends RestaurantsSlides
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RestaurantsSlidesAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("RestaurantsSlidesList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "RestaurantsSlidesList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "RestaurantsSlidesView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->restaurant_id->CurrentValue = null;
        $this->restaurant_id->OldValue = $this->restaurant_id->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'restaurant_id' first before field var 'x_restaurant_id'
        $val = $CurrentForm->hasValue("restaurant_id") ? $CurrentForm->getValue("restaurant_id") : $CurrentForm->getValue("x_restaurant_id");
        if (!$this->restaurant_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->restaurant_id->Visible = false; // Disable update for API request
            } else {
                $this->restaurant_id->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->restaurant_id->CurrentValue = $this->restaurant_id->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['restaurant_id'] = $this->restaurant_id->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->restaurant_id->EditValue = HtmlEncode($this->restaurant_id->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->restaurant_id->Required) {
            if (!$this->restaurant_id->IsDetailKey && EmptyValue($this->restaurant_id->FormValue)) {
                $this->restaurant_id->addErrorMessage(str_replace("%s", $this->restaurant_id->caption(), $this->restaurant_id->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->restaurant_id->FormValue)) {
            $this->restaurant_id->addErrorMessage($this->restaurant_id->getErrorMessage(false));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // restaurant_id
        $this->restaurant_id->setDbValueDef($rsnew, $this->restaurant_id->CurrentValue, 0, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("RestaurantsSlidesList"), "", $this->TableVar, true);
