 ...
 * Page class
 */
class EventOrdersList extends EventOrders
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EventOrdersList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fevent_orderslist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "EventOrdersAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "EventOrdersDelete";
        $this->MultiUpdateUrl = "EventOrdersUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fevent_orderslistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->event_time->AdvancedSearch->toJson(), ","); // Field event_time
        $filterList = Concat($filterList, $this->event_type->AdvancedSearch->toJson(), ","); // Field event_type
        $filterList = Concat($filterList, $this->pdf_link->AdvancedSearch->toJson(), ","); // Field pdf_link
        $filterList = Concat($filterList, $this->uploader->AdvancedSearch->toJson(), ","); // Field uploader
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fevent_orderslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field event_time
        $this->event_time->AdvancedSearch->SearchValue = @$filter["x_event_time"];
        $this->event_time->AdvancedSearch->SearchOperator = @$filter["z_event_time"];
        $this->event_time->AdvancedSearch->SearchCondition = @$filter["v_event_time"];
        $this->event_time->AdvancedSearch->SearchValue2 = @$filter["y_event_time"];
        $this->event_time->AdvancedSearch->SearchOperator2 = @$filter["w_event_time"];
        $this->event_time->AdvancedSearch->save();
        ...
        // Field pdf_link
        $this->pdf_link->AdvancedSearch->SearchValue = @$filter["x_pdf_link"];
        $this->pdf_link->AdvancedSearch->SearchOperator = @$filter["z_pdf_link"];
        $this->pdf_link->AdvancedSearch->SearchCondition = @$filter["v_pdf_link"];
        $this->pdf_link->AdvancedSearch->SearchValue2 = @$filter["y_pdf_link"];
        $this->pdf_link->AdvancedSearch->SearchOperator2 = @$filter["w_pdf_link"];
        $this->pdf_link->AdvancedSearch->save();

        // Field uploader
        $this->uploader->AdvancedSearch->SearchValue = @$filter["x_uploader"];
        $this->uploader->AdvancedSearch->SearchOperator = @$filter["z_uploader"];
        $this->uploader->AdvancedSearch->SearchCondition = @$filter["v_uploader"];
        $this->uploader->AdvancedSearch->SearchValue2 = @$filter["y_uploader"];
        $this->uploader->AdvancedSearch->SearchOperator2 = @$filter["w_uploader"];
        $this->uploader->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->pdf_link, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->uploader, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->data, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->progress, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->event_time); // event_time
            $this->updateSort($this->event_type); // event_type
            $this->updateSort($this->pdf_link); // pdf_link
            $this->updateSort($this->uploader); // uploader
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->event_time->setSort("");
                $this->event_type->setSort("");
                $this->pdf_link->setSort("");
                $this->uploader->setSort("");
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fevent_orderslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fevent_orderslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fevent_orderslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fevent_orderslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
