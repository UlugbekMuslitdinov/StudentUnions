 ...
 * Page class
 */
class FeedbackAdd extends Feedback
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FeedbackAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("FeedbackList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "FeedbackList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "FeedbackView") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->time->IsDetailKey) {
            ...
            $this->time->CurrentValue = UnFormatDateTime($this->time->CurrentValue, 0);
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->time->EditValue = HtmlEncode(FormatDateTime($this->time->CurrentValue, 8));
    ...
    protected function validateForm()
    {
        ...
        if (!CheckDate($this->time->FormValue)) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // time
        $this->time->setDbValueDef($rsnew, UnFormatDateTime($this->time->CurrentValue, 0), null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("FeedbackList"), "", $this->TableVar, true);
