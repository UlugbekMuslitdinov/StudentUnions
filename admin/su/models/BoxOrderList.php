 ...
 * Page class
 */
class BoxOrderList extends BoxOrder
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "BoxOrderList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fbox_orderlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "BoxOrderAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "BoxOrderDelete";
        $this->MultiUpdateUrl = "BoxOrderUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fbox_orderlistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->payment_idb->AdvancedSearch->toJson(), ","); // Field payment_idb
        $filterList = Concat($filterList, $this->amount_swipe->AdvancedSearch->toJson(), ","); // Field amount_swipe
        $filterList = Concat($filterList, $this->amount_total->AdvancedSearch->toJson(), ","); // Field amount_total
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fbox_orderlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field payment_idb
        $this->payment_idb->AdvancedSearch->SearchValue = @$filter["x_payment_idb"];
        $this->payment_idb->AdvancedSearch->SearchOperator = @$filter["z_payment_idb"];
        $this->payment_idb->AdvancedSearch->SearchCondition = @$filter["v_payment_idb"];
        $this->payment_idb->AdvancedSearch->SearchValue2 = @$filter["y_payment_idb"];
        $this->payment_idb->AdvancedSearch->SearchOperator2 = @$filter["w_payment_idb"];
        $this->payment_idb->AdvancedSearch->save();

        // Field amount_swipe
        $this->amount_swipe->AdvancedSearch->SearchValue = @$filter["x_amount_swipe"];
        $this->amount_swipe->AdvancedSearch->SearchOperator = @$filter["z_amount_swipe"];
        $this->amount_swipe->AdvancedSearch->SearchCondition = @$filter["v_amount_swipe"];
        $this->amount_swipe->AdvancedSearch->SearchValue2 = @$filter["y_amount_swipe"];
        $this->amount_swipe->AdvancedSearch->SearchOperator2 = @$filter["w_amount_swipe"];
        $this->amount_swipe->AdvancedSearch->save();

        // Field amount_total
        $this->amount_total->AdvancedSearch->SearchValue = @$filter["x_amount_total"];
        $this->amount_total->AdvancedSearch->SearchOperator = @$filter["z_amount_total"];
        $this->amount_total->AdvancedSearch->SearchCondition = @$filter["v_amount_total"];
        $this->amount_total->AdvancedSearch->SearchValue2 = @$filter["y_amount_total"];
        $this->amount_total->AdvancedSearch->SearchOperator2 = @$filter["w_amount_total"];
        $this->amount_total->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->payment_idb, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->payment_idb); // payment_idb
            $this->updateSort($this->amount_swipe); // amount_swipe
            $this->updateSort($this->amount_total); // amount_total
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->payment_idb->setSort("");
                $this->amount_swipe->setSort("");
                $this->amount_total->setSort("");
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbox_orderlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbox_orderlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fbox_orderlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fbox_orderlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
