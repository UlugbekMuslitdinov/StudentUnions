 ...
 * Page class
 */
class ExpressTimelineAdd extends ExpressTimeline
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'express_timeline';

    // Page object name
    public $PageObjName = "ExpressTimelineAdd";
    ...
    public function __construct()
    {
        ...
        // Table object (express_timeline)
        if (!isset($GLOBALS["express_timeline"]) || get_class($GLOBALS["express_timeline"]) == PROJECT_NAMESPACE . "express_timeline") {
            $GLOBALS["express_timeline"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'express_timeline');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("express_timeline"));
                $doc->Text = @$content;
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("ExpressTimelineList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ExpressTimelineList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ExpressTimelineView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->express_id->CurrentValue = null;
        $this->express_id->OldValue = $this->express_id->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'express_id' first before field var 'x_express_id'
        $val = $CurrentForm->hasValue("express_id") ? $CurrentForm->getValue("express_id") : $CurrentForm->getValue("x_express_id");
        if (!$this->express_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->express_id->Visible = false; // Disable update for API request
            } else {
                $this->express_id->setFormValue($val);
            }
        }

        // Check field name 'admin_id' first before field var 'x_admin_id'
        $val = $CurrentForm->hasValue("admin_id") ? $CurrentForm->getValue("admin_id") : $CurrentForm->getValue("x_admin_id");
        if (!$this->admin_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->admin_id->Visible = false; // Disable update for API request
            } else {
    ...
    {
        global $CurrentForm;
        $this->express_id->CurrentValue = $this->express_id->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['express_id'] = $this->express_id->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->express_id->EditValue = HtmlEncode($this->express_id->CurrentValue);
            ...
            $this->admin_id->EditValue = HtmlEncode($this->admin_id->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->express_id->Required) {
            if (!$this->express_id->IsDetailKey && EmptyValue($this->express_id->FormValue)) {
                $this->express_id->addErrorMessage(str_replace("%s", $this->express_id->caption(), $this->express_id->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->express_id->FormValue)) {
            $this->express_id->addErrorMessage($this->express_id->getErrorMessage(false));
        }
        if ($this->admin_id->Required) {
            if (!$this->admin_id->IsDetailKey && EmptyValue($this->admin_id->FormValue)) {
                $this->admin_id->addErrorMessage(str_replace("%s", $this->admin_id->caption(), $this->admin_id->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->admin_id->FormValue)) {
            $this->admin_id->addErrorMessage($this->admin_id->getErrorMessage(false));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // express_id
        $this->express_id->setDbValueDef($rsnew, $this->express_id->CurrentValue, null, false);

        // admin_id
        $this->admin_id->setDbValueDef($rsnew, $this->admin_id->CurrentValue, null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ExpressTimelineList"), "", $this->TableVar, true);
