...
// Page object
$CateringEventRequestsEdit = &$Page;
?>
<script>
...
var fcatering_event_requestsedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fcatering_event_requestsedit = currentForm = new ew.Form("fcatering_event_requestsedit", "edit");

    // Add fields
    var fields = ew.vars.tables.catering_event_requests.fields;
    fcatering_event_requestsedit.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fcatering_event_requestsedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    fcatering_event_requestsedit.validate = function () {
    ...
    // Form_CustomValidate
    fcatering_event_requestsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcatering_event_requestsedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fcatering_event_requestsedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fcatering_event_requestsedit" id="fcatering_event_requestsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_catering_event_requests_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_catering_event_requests_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="catering_event_requests" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fcatering_event_requestsedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fcatering_event_requestsedit", "x_created_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fcatering_event_requestsedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fcatering_event_requestsedit", "x_updated_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
