...
loadjs.ready("head", function () {
    ...
    fmealpackageadd = currentForm = new ew.Form("fmealpackageadd", "add");

    // Add fields
    var fields = ew.vars.tables.mealpackage.fields;
    fmealpackageadd.addFields([
    ...
    // Validate form
    fmealpackageadd.validate = function () {
    ...
    // Form_CustomValidate
    fmealpackageadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmealpackageadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fmealpackageadd.lists.water = <?= $Page->water->toClientList($Page) ?>;
    loadjs.done("fmealpackageadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fmealpackageadd" id="fmealpackageadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->water->isInvalidClass() ?>" data-table="mealpackage" data-field="x_water" name="x_water[]" id="x_water_137345" value="1"<?= ConvertToBool($Page->water->CurrentValue) ? " checked" : "" ?><?= $Page->water->editAttributes() ?> aria-describedby="x_water_help">
    <label class="custom-control-label" for="x_water_137345"></label>
</div>
...
<script>
loadjs.ready(["fmealpackageadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fmealpackageadd", "x_timestamp", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
