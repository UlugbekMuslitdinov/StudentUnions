...
var fcatering_highland_menuedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fcatering_highland_menuedit = currentForm = new ew.Form("fcatering_highland_menuedit", "edit");

    // Add fields
    var fields = ew.vars.tables.catering_highland_menu.fields;
    fcatering_highland_menuedit.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fcatering_highland_menuedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    fcatering_highland_menuedit.validate = function () {
    ...
    // Form_CustomValidate
    fcatering_highland_menuedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcatering_highland_menuedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fcatering_highland_menuedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fcatering_highland_menuedit" id="fcatering_highland_menuedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_catering_highland_menu_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_catering_highland_menu_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="catering_highland_menu" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
