...
loadjs.ready("head", function () {
    ...
    fbox_choiceadd = currentForm = new ew.Form("fbox_choiceadd", "add");

    // Add fields
    var fields = ew.vars.tables.box_choice.fields;
    fbox_choiceadd.addFields([
    ...
    // Validate form
    fbox_choiceadd.validate = function () {
    ...
    // Form_CustomValidate
    fbox_choiceadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fbox_choiceadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fbox_choiceadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fbox_choiceadd" id="fbox_choiceadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<script>
loadjs.ready(["fbox_choiceadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fbox_choiceadd", "x_pickup_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
