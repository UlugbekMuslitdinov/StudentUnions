...
loadjs.ready("head", function () {
    ...
    // Validate form
    fsession_handleradd.validate = function () {
    ...
    // Form_CustomValidate
    fsession_handleradd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsession_handleradd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fsession_handleradd.lists.deposit_to = <?= $Page->deposit_to->toClientList($Page) ?>;
    loadjs.done("fsession_handleradd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fsession_handleradd" id="fsession_handleradd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->deposit_to->isInvalidClass() ?>" data-table="session_handler" data-field="x_deposit_to" name="x_deposit_to[]" id="x_deposit_to_623406" value="1"<?= ConvertToBool($Page->deposit_to->CurrentValue) ? " checked" : "" ?><?= $Page->deposit_to->editAttributes() ?> aria-describedby="x_deposit_to_help">
    <label class="custom-control-label" for="x_deposit_to_623406"></label>
</div>
...
<script>
loadjs.ready(["fsession_handleradd", "datetimepicker"], function() {
    ew.createDateTimePicker("fsession_handleradd", "x_timestamp", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
