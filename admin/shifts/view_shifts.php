...
<body>
	...
	<div class="col-12 major_heading">
		<!--Shift-->
	</div>
	<?php
	...
	<div class="text">
		<a href="view_shifts.php?id=<?= $row['id'] ?>"><span class="shift_date"><?= $row['shift_date'] ?></span> (<?= $row['time_from'] ?> - <?= $row['time_to'] ?>) at <span class="location"><?= $row['location'] ?></span></a> (<?= $row['event'] ?>) - <span class="num_positions"><?= $row['num_positions'] ?></span> Avalilable <?php if ($row['urgent'] == 1) { ?> &nbsp;&nbsp;&nbsp;<img src="images/urgent.png" height="40px" /> <?php } ?>
	</div>

	<!-- List of applied names 	-->
	<?php
	// Find Employees applied for each shift.
	$query_employees = "SELECT shift_id, employee_id, status, E.* 
						FROM Shifts S 
						LEFT JOIN Signups SU ON S.id = SU.shift_id 
						LEFT JOIN Employees E ON SU.employee_id = E.id 
						WHERE shift_id = " . $row['id'] . " ORDER BY first_name, last_name";
				...
				<tbody>
					<tr>
						<td><img src="images/bullet.jfif" width="10px;" />&nbsp;&nbsp;<a href="employee_view.php?id=<?= $row2['employee_id'] ?>"><span class="text_bold"><u><?= $row2['first_name'] ?> <?= $row2['last_name'] ?></u></span></a> (at <span class="filled"><?= $row2['affiliation'] ?></span>)
							...
							<?php if ((isset($row2['banned'])) && (($row2['banned']) == "Yes")) { ?><img src="images/banned.jpg" alt="Banned" height="30px;" />
						...
						<tbody>
							<tr>
								...
								<td width="100px">
									...
									<?php if (($row3['attendance'] == "Excused") || ($row3['attendance'] == "No Show")) { ?> <span class="text_bold_blue"><?= $row3['attendance'] ?></span> <?php } elseif ($row3['status'] == "Approved") { ?> <button class="approve"><a href="mark_excused.php?shift_id=<?= $row2['shift_id'] ?>&employee_id=<?= $row2['employee_id'] ?>&task_id=<?= $row3['task_id'] ?>">EXCUSED</a></button> <?php } ?>
