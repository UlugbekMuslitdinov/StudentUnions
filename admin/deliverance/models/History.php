...
class History extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // id
        $this->id = new DbField('history', 'history', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'NO');
        ...
        // netID
        $this->netID = new DbField('history', 'history', 'x_netID', 'netID', '`netID`', '`netID`', 200, 25, -1, false, '`netID`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // action
        $this->_action = new DbField('history', 'history', 'x__action', 'action', '`action`', '`action`', 200, 25, -1, false, '`action`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->_action->Sortable = true; // Allow sort
        $this->Fields['action'] = &$this->_action;

        // server
        $this->server = new DbField('history', 'history', 'x_server', 'server', '`server`', '`server`', 200, 15, -1, false, '`server`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->server->Nullable = false; // NOT NULL field
        $this->server->Required = true; // Required field
        $this->server->Sortable = true; // Allow sort
        $this->Fields['server'] = &$this->server;

        // page
        $this->_page = new DbField('history', 'history', 'x__page', 'page', '`page`', '`page`', 200, 50, -1, false, '`page`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->_page->Sortable = true; // Allow sort
        $this->Fields['page'] = &$this->_page;

        // resourceName
        $this->resourceName = new DbField('history', 'history', 'x_resourceName', 'resourceName', '`resourceName`', '`resourceName`', 200, 100, -1, false, '`resourceName`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->resourceName->Sortable = true; // Allow sort
        $this->Fields['resourceName'] = &$this->resourceName;

        // filePath
        $this->filePath = new DbField('history', 'history', 'x_filePath', 'filePath', '`filePath`', '`filePath`', 200, 255, -1, false, '`filePath`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->filePath->Sortable = true; // Allow sort
        $this->Fields['filePath'] = &$this->filePath;

        // site
        $this->site = new DbField('history', 'history', 'x_site', 'site', '`site`', '`site`', 200, 50, -1, false, '`site`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->site->Sortable = true; // Allow sort
        $this->Fields['site'] = &$this->site;

        // timestamp
        $this->timestamp = new DbField('history', 'history', 'x_timestamp', 'timestamp', '`timestamp`', '`timestamp`', 3, 11, -1, false, '`timestamp`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        $this->timestamp->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`history`";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->server->DbValue = $row['server'];
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "HistoryEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "HistoryAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("HistoryView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("HistoryView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "HistoryAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "HistoryAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("HistoryEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("HistoryAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("HistoryDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $this->server->EditValue = $this->server->CurrentValue;
