 ...
 * Page class
 */
class HistoryEdit extends History
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "HistoryEdit";
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // netID
            $this->netID->setDbValueDef($rsnew, $this->netID->CurrentValue, "", $this->netID->ReadOnly);

            // action
            $this->_action->setDbValueDef($rsnew, $this->_action->CurrentValue, null, $this->_action->ReadOnly);

            // server
            $this->server->setDbValueDef($rsnew, $this->server->CurrentValue, "", $this->server->ReadOnly);

            // page
            $this->_page->setDbValueDef($rsnew, $this->_page->CurrentValue, null, $this->_page->ReadOnly);

            // resourceName
            $this->resourceName->setDbValueDef($rsnew, $this->resourceName->CurrentValue, null, $this->resourceName->ReadOnly);

            // filePath
            $this->filePath->setDbValueDef($rsnew, $this->filePath->CurrentValue, null, $this->filePath->ReadOnly);

            // site
            $this->site->setDbValueDef($rsnew, $this->site->CurrentValue, null, $this->site->ReadOnly);

            // timestamp
            $this->timestamp->setDbValueDef($rsnew, $this->timestamp->CurrentValue, 0, $this->timestamp->ReadOnly);
