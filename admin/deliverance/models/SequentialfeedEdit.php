 ...
 * Page class
 */
class SequentialfeedEdit extends Sequentialfeed
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'sequentialfeed';

    // Page object name
    public $PageObjName = "SequentialfeedEdit";
    ...
    public function __construct()
    {
        ...
        // Table object (sequentialfeed)
        if (!isset($GLOBALS["sequentialfeed"]) || get_class($GLOBALS["sequentialfeed"]) == PROJECT_NAMESPACE . "sequentialfeed") {
            $GLOBALS["sequentialfeed"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sequentialfeed');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("sequentialfeed"));
                $doc->Text = @$content;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("SequentialfeedList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "SequentialfeedList") {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // resourceID
            $this->resourceID->setDbValueDef($rsnew, $this->resourceID->CurrentValue, 0, $this->resourceID->ReadOnly);

            // displayBlockID
            $this->displayBlockID->setDbValueDef($rsnew, $this->displayBlockID->CurrentValue, 0, $this->displayBlockID->ReadOnly);

            // startDate
            $this->startDate->setDbValueDef($rsnew, UnFormatDateTime($this->startDate->CurrentValue, 0), CurrentDate(), $this->startDate->ReadOnly);

            // endDate
            $this->endDate->setDbValueDef($rsnew, UnFormatDateTime($this->endDate->CurrentValue, 0), CurrentDate(), $this->endDate->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("SequentialfeedList"), "", $this->TableVar, true);
        $pageId = "edit";
