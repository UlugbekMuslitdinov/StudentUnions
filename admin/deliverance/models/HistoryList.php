...
class HistoryList extends History
{
    ...
    // Page object name
    public $PageObjName = "HistoryList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fhistorylist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "HistoryAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "HistoryDelete";
        $this->MultiUpdateUrl = "HistoryUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fhistorylistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->server->AdvancedSearch->toJson(), ","); // Field server
        $filterList = Concat($filterList, $this->_page->AdvancedSearch->toJson(), ","); // Field page
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fhistorylistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field server
        $this->server->AdvancedSearch->SearchValue = @$filter["x_server"];
        $this->server->AdvancedSearch->SearchOperator = @$filter["z_server"];
        $this->server->AdvancedSearch->SearchCondition = @$filter["v_server"];
        $this->server->AdvancedSearch->SearchValue2 = @$filter["y_server"];
        $this->server->AdvancedSearch->SearchOperator2 = @$filter["w_server"];
        $this->server->AdvancedSearch->save();

        // Field page
        $this->_page->AdvancedSearch->SearchValue = @$filter["x__page"];
        $this->_page->AdvancedSearch->SearchOperator = @$filter["z__page"];
        $this->_page->AdvancedSearch->SearchCondition = @$filter["v__page"];
        $this->_page->AdvancedSearch->SearchValue2 = @$filter["y__page"];
        $this->_page->AdvancedSearch->SearchOperator2 = @$filter["w__page"];
        $this->_page->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->server, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->_page, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->server); // server
            $this->updateSort($this->_page); // page
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fhistorylistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fhistorylistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fhistorylist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fhistorylistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
