...
switch($action){
	...
	case 'search':
		$itemsPerPage = 30;
		$count = 'select COUNT(distinct emplid) as total from applications
			inner join schedules on applications.application_id = schedules.application_id
			left join applying_for on applications.application_id = applying_for.application_id
			left join work_history on applications.application_id = work_history.application_id
			where active=1
			and app_id=1
			and semester_id='.$_POST['semester'];

		if($_POST['work_exp'] == "true"){
			$count .= ' and work_history_id is NOT NULL';
		}

		if($_POST['hss'] == "true"){
			$count .=' and studentType <> "high_school_student"';
		}
		if($_POST['freshman'] == "true"){
			$count .=' and classStanding <> "Freshman"';
		}

		if($_POST['sophomore'] == "true"){
			$count .=' and classStanding <> "Sophomore"';
		}

		if($_POST['junior'] == "true"){
			$count .=' and classStanding <> "Junior"';
		}

		if($_POST['senior'] == "true"){
			$count .=' and classStanding <> "Senior"';
		}

		if($_POST['grad'] == "true"){
			$count .=' and classStanding <> "Grad Student"';
		}

		if($_POST['fname'] != ''){
			$count .= ' and firstName like "'.$_POST['fname'].'%"';
		}

		if($_POST['lname'] != ''){
			$count .= ' and lastName like "'.$_POST['lname'].'%"';
		}

		if($_POST['workStudy'] == "true"){
			$count .= ' and workStudy = 1';
		}

		$num = mysqli_fetch_assoc($db->query($count));

		$pages = ceil($num['total']/$itemsPerPage);
		$pagination = '';
		if($pages > 1)
		{
		    ...
		    for($p = 0; $p<$pages; $p++)
		    {
		        $pagination .= '<li ' . ($_POST['page'] == $p ? "class='active'" : ""). '><a href="#" class="paginate_click" id="'.$p.'-page">'.$p.'</a></li>';
		    }
		    $pagination .= '</ul>';
		}
		...
		$position = ($page_number * $itemsPerPage);
		...
		//var_dump($days);
		$query = 'select distinct applications.*, schedules.*, (BIT_COUNT(mon & '.$days[0][0].') + BIT_COUNT(mon2 & '.$days[0][1].') + BIT_COUNT(tue & '.$days[1][0].') + BIT_COUNT(tue2 & '.$days[1][1].') + BIT_COUNT(wed & '.$days[2][0].') + BIT_COUNT(wed2 & '.$days[2][1].') + BIT_COUNT(thu & '.$days[3][0].') + BIT_COUNT(thu2 & '.$days[3][1].') + BIT_COUNT(fri & '.$days[4][0].') + BIT_COUNT(fri2 & '.$days[4][1].') + BIT_COUNT(sat & '.$days[5][0].') +BIT_COUNT( sat2 & '.$days[5][1].') + BIT_COUNT(sun & '.$days[6][0].') + BIT_COUNT(sun2 & '.$days[6][1].')) as matchdegree from applications inner join schedules using(application_id) left join applying_for using(application_id) left join work_history using (application_id) where active=1 and app_id=1 and semester_id='.$_POST['semester'];
		...
		// $query .= ' LIMIT ' . $position . ', ' . $itemsPerPage;
		...
		if(!is_array($apps)){
			...
			echo $pagination . "<br>";
			echo $position . " - " . ($results->num_rows + $position) . " of " . $num['total'];
		?>
				...
				echo '<tr >';
					echo '<td><div>';
						...
						foreach($app['jobs'] as $job){
							...
							}else if($job['type'] == 'any'){
								echo "<label>Any Position</label>";
							}else{
						...
						$resume = mysqli_fetch_assoc($db->query('select resume_id from resumes where application_id ='.$app['application_id']));
						if($resume['resume_id'])
							echo '<a href="resume.php?id='.$resume['resume_id'].'">Download Resume</a><br />';
						...
						echo '<div class="btn-group btn-group-justified">';
							if($app['hired'] != 1){
								if($app['saved'] != 1){
									echo '<div class="btn-group"><input type="button" class="btn btn-primary" value="Save App" onclick="save_app(this,'.$app['application_id'].',0,' .(!is_null($job['unit_id']) ? $job['unit_id'] : $_SESSION['user']['unit_id']) .')"/></div>';
								}else{
