...
$filename = "EmploymentPopupResponses_2022ByMonth"; // File Name
...
for ($i=0; $i<6; $i++) {
	...
	for ($j=1; $j<13; $j++) {
		...
		$user_query = $db->query("SELECT count(response) as total FROM employment WHERE response='" . $responseTypes[$i] . "' AND timestamp LIKE '2022-" . $j . "%';");
		$user_query = $user_query->fetch_assoc();
		// Write data to file
		echo strval($user_query['total']) . ",";
		//get month total for all response types
		$monthsTotal = $db->query("SELECT count(response) as total FROM employment WHERE timestamp LIKE '2022-" . $j . "%'"); //should look like ('01'=>'some int', '02'=>'some int'... '12'=>'some int')
