...
$page_options['page'] = 'View Transactions - 50/50';
...
if(isset($_SESSION['5050mp_cust']['state']) && $_SESSION['5050mp_cust']['state'] == 'pending'){
...
if(!isset($_GET['page'])){
	...
	$total = getMPTransactionCount($_SESSION['5050mp_cust']['id']);
	$pages = ceil($total/RESULTS_PAGE);
	if ($total > 0)
	{
		$transactions = getMPTransactions($_SESSION['5050mp_cust']['id'], $rowstart, $rowend);
	}
...
<h1>50/50 Transaction History</h1>
...
if($pages > 1){
						...
						?>
							...
							<?php
								if($current_page!=1){
									print '<a href="viewtransactions5050.php?page='.($current_page-1).'" class="page-link" >&lt; Previous</a> ';
								}
							?>

								<?php for($x=max(1, $current_page-3); $x<min($current_page+4, $pages+1); $x++){?>
								<a href="viewtransactions5050.php?page=<?=$x?>" class="page-link" >
									<?=($x==$current_page)?'&nbsp;<b style="color:#363636;">'.($x).'</b>':'&nbsp;'.($x)?>
								</a>
								<?php }?>
							<?php
								if($current_page!=$pages){
									print '<a href="viewtransactions5050.php?page='.($current_page+1).'" class="page-link" >Next &gt;</a> ';
								}
							?>
