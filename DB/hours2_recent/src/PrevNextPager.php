...
class PrevNextPager extends Pager
{
    ...
    public function render()
    {
        ...
        if ($this->isVisible()) {
            ...
            $pageNumber = '<!-- current page number --><input class="form-control ew-page-no" type="text" data-pagesize="' . $this->PageSize . '" data-pagecount="' . $this->PageCount . '" name="' . $tablePageNo . '" value="' . $formatInteger($this->CurrentPageNumber) . '"' . ($this->Modal ? " disabled" : "") . '>';
            $html = <<<PAGER
                <div class="ew-pager">
