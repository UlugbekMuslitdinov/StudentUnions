...
/**
 * Table class for begal.csv
 */
class Begalcsv extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'begalcsv';
        $this->TableName = 'begal.csv';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`begal.csv`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->D->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        ...
        $this->F->InputTextType = "text";
        $this->F->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`begal.csv`";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("BegalcsvList");
    }
    ...
    {
        global $Language;
        if ($pageName == "BegalcsvView") {
            return $Language->phrase("View");
        } elseif ($pageName == "BegalcsvEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "BegalcsvAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("BegalcsvView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("BegalcsvView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "BegalcsvAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "BegalcsvAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("BegalcsvEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BegalcsvAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("BegalcsvDelete", $this->getUrlParm());
    }
    ...
    public function renderEditRow()
    {
        ...
        // B
        $this->B->setupEditAttributes();
        $this->B->EditCustomAttributes = "";
        if (!$this->B->Raw) {
            $this->B->CurrentValue = HtmlDecode($this->B->CurrentValue);
        }
        $this->B->EditValue = $this->B->CurrentValue;
        $this->B->PlaceHolder = RemoveHtml($this->B->caption());

        // C
        $this->C->setupEditAttributes();
        ...
        if (strval($this->C->EditValue) != "" && is_numeric($this->C->EditValue)) {
            $this->C->EditValue = FormatNumber($this->C->EditValue, null);
        }

        // D
        $this->D->setupEditAttributes();
        ...
        if (strval($this->D->EditValue) != "" && is_numeric($this->D->EditValue)) {
            $this->D->EditValue = FormatNumber($this->D->EditValue, null);
        }

        // E
        $this->E->setupEditAttributes();
        ...
        // F
        $this->F->setupEditAttributes();
        $this->F->EditCustomAttributes = "";
        $this->F->EditValue = $this->F->CurrentValue;
        $this->F->PlaceHolder = RemoveHtml($this->F->caption());
        if (strval($this->F->EditValue) != "" && is_numeric($this->F->EditValue)) {
            $this->F->EditValue = FormatNumber($this->F->EditValue, null);
        }
