 ...
 * Page class
 */
class ExceptionsAdd extends Exceptions
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'exceptions';

    // Page object name
    public $PageObjName = "ExceptionsAdd";
    ...
    public function __construct()
    {
        ...
        // Table object (exceptions)
        if (!isset($GLOBALS["exceptions"]) || get_class($GLOBALS["exceptions"]) == PROJECT_NAMESPACE . "exceptions") {
            $GLOBALS["exceptions"] = &$this;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'exceptions');
        }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("ExceptionsList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ExceptionsList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ExceptionsView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->date_of->CurrentValue = null;
        $this->date_of->OldValue = $this->date_of->CurrentValue;
        $this->open->CurrentValue = null;
        $this->open->OldValue = $this->open->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'date_of' first before field var 'x_date_of'
        $val = $CurrentForm->hasValue("date_of") ? $CurrentForm->getValue("date_of") : $CurrentForm->getValue("x_date_of");
        if (!$this->date_of->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->date_of->Visible = false; // Disable update for API request
            } else {
                $this->date_of->setFormValue($val, true, $validate);
            }
            $this->date_of->CurrentValue = UnFormatDateTime($this->date_of->CurrentValue, $this->date_of->formatPattern());
        }
        ...
        if (!$this->open->IsDetailKey) {
            ...
            $this->open->CurrentValue = UnFormatDateTime($this->open->CurrentValue, $this->open->formatPattern());
        }
        ...
        if (!$this->close->IsDetailKey) {
            ...
            $this->close->CurrentValue = UnFormatDateTime($this->close->CurrentValue, $this->close->formatPattern());
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->date_of->CurrentValue = $this->date_of->FormValue;
        $this->date_of->CurrentValue = UnFormatDateTime($this->date_of->CurrentValue, $this->date_of->formatPattern());
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->date_of->setDbValue($row['date_of']);
    ...
    protected function newRow()
    {
        ...
        $row['date_of'] = $this->date_of->CurrentValue;
        $row['open'] = $this->open->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // date_of
            $this->date_of->ViewValue = $this->date_of->CurrentValue;
            $this->date_of->ViewValue = FormatDateTime($this->date_of->ViewValue, 0);
            $this->date_of->ViewCustomAttributes = "";

            // open
            $this->open->ViewValue = $this->open->CurrentValue;
            $this->open->ViewValue = FormatDateTime($this->open->ViewValue, 4);
            $this->open->ViewCustomAttributes = "";

            // close
            $this->close->ViewValue = $this->close->CurrentValue;
            $this->close->ViewValue = FormatDateTime($this->close->ViewValue, 4);
            ...
            // date_of
            $this->date_of->LinkCustomAttributes = "";
            $this->date_of->HrefValue = "";
            ...
            // date_of
            $this->date_of->setupEditAttributes();
            $this->date_of->EditCustomAttributes = "";
            $this->date_of->EditValue = HtmlEncode(FormatDateTime($this->date_of->CurrentValue, 8));
            $this->date_of->PlaceHolder = RemoveHtml($this->date_of->caption());

            // open
            $this->open->setupEditAttributes();
            $this->open->EditCustomAttributes = "";
            $this->open->EditValue = HtmlEncode(FormatDateTime($this->open->CurrentValue, 4));
            $this->open->PlaceHolder = RemoveHtml($this->open->caption());

            // close
            $this->close->setupEditAttributes();
            $this->close->EditCustomAttributes = "";
            $this->close->EditValue = HtmlEncode(FormatDateTime($this->close->CurrentValue, 4));
    ...
    protected function validateForm()
    {
        ...
        if ($this->date_of->Required) {
            if (!$this->date_of->IsDetailKey && EmptyValue($this->date_of->FormValue)) {
                $this->date_of->addErrorMessage(str_replace("%s", $this->date_of->caption(), $this->date_of->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->date_of->FormValue, $this->date_of->formatPattern())) {
            $this->date_of->addErrorMessage($this->date_of->getErrorMessage(false));
        }
        ...
        if (!CheckTime($this->open->FormValue, $this->open->formatPattern())) {
        ...
        if (!CheckTime($this->close->FormValue, $this->close->formatPattern())) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // date_of
        $this->date_of->setDbValueDef($rsnew, UnFormatDateTime($this->date_of->CurrentValue, $this->date_of->formatPattern()), null, false);

        // open
        $this->open->setDbValueDef($rsnew, UnFormatDateTime($this->open->CurrentValue, $this->open->formatPattern()), null, false);

        // close
        $this->close->setDbValueDef($rsnew, UnFormatDateTime($this->close->CurrentValue, $this->close->formatPattern()), null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ExceptionsList"), "", $this->TableVar, true);
