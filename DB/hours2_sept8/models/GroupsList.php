...
class GroupsList extends Groups
{
    ...
    public function __construct()
    {
        ...
        // Table object (groups)
        if (!isset($GLOBALS["groups"]) || get_class($GLOBALS["groups"]) == PROJECT_NAMESPACE . "groups") {
            $GLOBALS["groups"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "GroupsAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "GroupsDelete";
        $this->MultiUpdateUrl = "GroupsUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'groups');
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->group_key->AdvancedSearch->toJson(), ","); // Field group_key
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fgroupssrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field group_key
        $this->group_key->AdvancedSearch->SearchValue = @$filter["x_group_key"];
        $this->group_key->AdvancedSearch->SearchOperator = @$filter["z_group_key"];
        $this->group_key->AdvancedSearch->SearchCondition = @$filter["v_group_key"];
        $this->group_key->AdvancedSearch->SearchValue2 = @$filter["y_group_key"];
        $this->group_key->AdvancedSearch->SearchOperator2 = @$filter["w_group_key"];
        $this->group_key->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->active;
        $searchFlds[] = &$this->group_key;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->group_key); // group_key
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fgroupslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fgroupslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->group_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("group_key", $this->createColumnOption("group_key"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fgroupssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fgroupssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fgroupslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->group_key->setDbValue($row['group_key']);
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // group_key
            $this->group_key->ViewValue = $this->group_key->CurrentValue;
            $this->group_key->ViewCustomAttributes = "";
            ...
            // group_key
            $this->group_key->LinkCustomAttributes = "";
            $this->group_key->HrefValue = "";
            $this->group_key->TooltipValue = "";
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-bs-toggle=\"button\" data-form=\"fgroupssrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
