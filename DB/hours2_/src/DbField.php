...
class DbField
{
    ...
    {
        if (in_array($this->DataType, [DATATYPE_DATE, DATATYPE_TIME])) {
            return $fmt = $this->formatPattern();
        }
        return "";
    }
    ...
    public function selectOptionListHtml($name = "", $multiple = null)
    {
        ...
        $str = "";
        $multiple = $multiple ?? $this->SelectMultiple;
        if ($multiple) {
    ...
    public function setDbValueDef(&$row, $value, $default, $skip = false)
    {
        ...
        switch ($this->Type) {
            ...
            case 17:
            case 18: // Integer
                $value = ParseInteger(trim($value), "", \NumberFormatter::TYPE_INT32);
                $dbValue = $value !== false ? $value : $default;
                break;
            case 19:
            case 20:
            case 21: // Big integer
                $value = ParseInteger(trim($value));
                $dbValue = $value !== false ? $value : $default;
                break;
            case 5:
            case 6:
