 ...
 * Page class
 */
class LocationList extends Location
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LocationList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "LocationAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "LocationDelete";
        $this->MultiUpdateUrl = "LocationUpdate";
    ...
    public function run()
    {
        ...
        $this->location_url->Visible = false;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->group_hours); // group_hours
            $this->updateSort($this->old_id); // old_id
            $this->updateSort($this->accept_plus_discount); // accept_plus_discount
            $this->updateSort($this->breakfast); // breakfast
            $this->updateSort($this->lunch); // lunch
            $this->updateSort($this->dinner); // dinner
            $this->updateSort($this->continuous); // continuous
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->location_url->setSort("");
                $this->group_id->setSort("");
                $this->active->setSort("");
                $this->group_hours->setSort("");
                $this->phone->setSort("");
                $this->subgroup->setSort("");
                $this->old_id->setSort("");
                $this->short_name->setSort("");
                $this->accept_plus_discount->setSort("");
                $this->lat->setSort("");
                $this->long->setSort("");
                $this->ua_mobile_categories->setSort("");
                $this->breakfast->setSort("");
                $this->lunch->setSort("");
                $this->dinner->setSort("");
                $this->continuous->setSort("");
                $this->hours_message->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"flocationlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"flocationlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->location_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("group_hours", $this->createColumnOption("group_hours"));
            $option->add("old_id", $this->createColumnOption("old_id"));
            $option->add("accept_plus_discount", $this->createColumnOption("accept_plus_discount"));
            $option->add("breakfast", $this->createColumnOption("breakfast"));
            $option->add("lunch", $this->createColumnOption("lunch"));
            $option->add("dinner", $this->createColumnOption("dinner"));
            $option->add("continuous", $this->createColumnOption("continuous"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"flocationsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"flocationsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="flocationlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
