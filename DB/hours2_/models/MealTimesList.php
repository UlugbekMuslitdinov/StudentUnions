 ...
 * Page class
 */
class MealTimesList extends MealTimes
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MealTimesList";
    ...
    public $FormName = "fmeal_timeslist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "MealTimesAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "MealTimesDelete";
        $this->MultiUpdateUrl = "MealTimesUpdate";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->meal_details_id); // meal_details_id
            $this->updateSort($this->startm); // startm
            $this->updateSort($this->endm); // endm
            $this->updateSort($this->startt); // startt
            $this->updateSort($this->endt); // endt
            $this->updateSort($this->startw); // startw
            $this->updateSort($this->endw); // endw
            $this->updateSort($this->startr); // startr
            $this->updateSort($this->endr); // endr
            $this->updateSort($this->startf); // startf
            $this->updateSort($this->endf); // endf
            $this->updateSort($this->starts); // starts
            $this->updateSort($this->ends); // ends
            $this->updateSort($this->startu); // startu
            $this->updateSort($this->endu); // endu
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmeal_timeslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmeal_timeslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("meal_details_id", $this->createColumnOption("meal_details_id"));
            $option->add("startm", $this->createColumnOption("startm"));
            $option->add("endm", $this->createColumnOption("endm"));
            $option->add("startt", $this->createColumnOption("startt"));
            $option->add("endt", $this->createColumnOption("endt"));
            $option->add("startw", $this->createColumnOption("startw"));
            $option->add("endw", $this->createColumnOption("endw"));
            $option->add("startr", $this->createColumnOption("startr"));
            $option->add("endr", $this->createColumnOption("endr"));
            $option->add("startf", $this->createColumnOption("startf"));
            $option->add("endf", $this->createColumnOption("endf"));
            $option->add("starts", $this->createColumnOption("starts"));
            $option->add("ends", $this->createColumnOption("ends"));
            $option->add("startu", $this->createColumnOption("startu"));
            $option->add("endu", $this->createColumnOption("endu"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmeal_timessrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmeal_timessrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmeal_timeslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
