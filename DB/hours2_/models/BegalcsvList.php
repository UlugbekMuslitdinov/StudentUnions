 ...
 * Page class
 */
class BegalcsvList extends Begalcsv
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'begal.csv';

    // Page object name
    public $PageObjName = "BegalcsvList";
    ...
    public function __construct()
    {
        ...
        // Table object (begalcsv)
        if (!isset($GLOBALS["begalcsv"]) || get_class($GLOBALS["begalcsv"]) == PROJECT_NAMESPACE . "begalcsv") {
            $GLOBALS["begalcsv"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "BegalcsvAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "BegalcsvDelete";
        $this->MultiUpdateUrl = "BegalcsvUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'begal.csv');
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->B->AdvancedSearch->toJson(), ","); // Field B
        $filterList = Concat($filterList, $this->C->AdvancedSearch->toJson(), ","); // Field C
        $filterList = Concat($filterList, $this->D->AdvancedSearch->toJson(), ","); // Field D
        $filterList = Concat($filterList, $this->E->AdvancedSearch->toJson(), ","); // Field E
        $filterList = Concat($filterList, $this->F->AdvancedSearch->toJson(), ","); // Field F
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fbegalcsvsrch", $filters);
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbegalcsvlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbegalcsvlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("A", $this->createColumnOption("A"));
            $option->add("B", $this->createColumnOption("B"));
            $option->add("C", $this->createColumnOption("C"));
            $option->add("D", $this->createColumnOption("D"));
            $option->add("E", $this->createColumnOption("E"));
            $option->add("F", $this->createColumnOption("F"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbegalcsvsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbegalcsvsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fbegalcsvlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->C->ViewValue = FormatNumber($this->C->ViewValue, "");
            $this->C->ViewCustomAttributes = "";

            // D
            $this->D->ViewValue = $this->D->CurrentValue;
            $this->D->ViewValue = FormatNumber($this->D->ViewValue, "");
            ...
            $this->F->ViewValue = FormatNumber($this->F->ViewValue, "");
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-bs-toggle=\"button\" data-form=\"fbegalcsvsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
