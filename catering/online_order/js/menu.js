...
var extra_price = [5.99,5.99,5.99,6.00];
...
$('body').on('click','#reviewOrder',function(){
	...
	if (pack_upgrade_selected != party_pack_upgrade){
		...
		$("#partyPack_upgrade_err").text('Select two flavors per Party Pack');
	}

	// Check if there are any negative inputs
	if($("#party_pack_upgrade").val() < 0 || $("#extra_salsa").val() < 0 || $("#extra_guacamole").val() < 0 || $("#extra_sourcream").val() < 0) {
		return;
	}
