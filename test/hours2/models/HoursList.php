...
class HoursList extends Hours
{
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "HoursAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "HoursDelete";
        $this->MultiUpdateUrl = "HoursUpdate";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->openm); // openm
            $this->updateSort($this->closem); // closem
            $this->updateSort($this->opent); // opent
            $this->updateSort($this->closet); // closet
            $this->updateSort($this->openw); // openw
            $this->updateSort($this->closew); // closew
            $this->updateSort($this->openr); // openr
            $this->updateSort($this->closer); // closer
            $this->updateSort($this->openf); // openf
            $this->updateSort($this->closef); // closef
            $this->updateSort($this->opens); // opens
            $this->updateSort($this->closes); // closes
            $this->updateSort($this->openu); // openu
            $this->updateSort($this->closeu); // closeu
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fhourslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fhourslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("openm", $this->createColumnOption("openm"));
            $option->add("closem", $this->createColumnOption("closem"));
            $option->add("opent", $this->createColumnOption("opent"));
            $option->add("closet", $this->createColumnOption("closet"));
            $option->add("openw", $this->createColumnOption("openw"));
            $option->add("closew", $this->createColumnOption("closew"));
            $option->add("openr", $this->createColumnOption("openr"));
            $option->add("closer", $this->createColumnOption("closer"));
            $option->add("openf", $this->createColumnOption("openf"));
            $option->add("closef", $this->createColumnOption("closef"));
            $option->add("opens", $this->createColumnOption("opens"));
            $option->add("closes", $this->createColumnOption("closes"));
            $option->add("openu", $this->createColumnOption("openu"));
            $option->add("closeu", $this->createColumnOption("closeu"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fhourssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fhourssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fhourslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
