...
function checkShift(id) {	
	...
	console.log('checkShift', document.getElementById("shift_" + id));
	if (isChecked === true) {
	...
	function selectTasks(shiftid, taskid, stid) {
		...
		console.log('selectTasks', document.getElementById("task_" + stid));
		if (isChecked === true) {
			var checked = 1; // Display Requirements if checked.
		} else {
			var checked = 0; // Hide Requirements if unchecked.
		...
		var myUpdater = new Ajax.Updater(placeholder, url, {
			method: 'get',
			parameters: postData
		});
	}
	...
	function selectFulfill(id) {
		...
		console.log(requirement_checkbox);
...
<body onload='setFocusToTextBox()'>
		...
		<form class="form-inline" action="insert_signup.php" method="POST" onSubmit="return checkForm();">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="5">
				<tbody>
					<tr>
						<td width="300">
							<div class="text_bold">FIRST NAME <span class="asterisk">*</span></div>
						</td>
						<td><input type="text" class="text" name="first_name" id="first_name" size="40" value="<?php echo $first_name; ?>" required></td>
					</tr>
					<tr>
						<td>
							<div class="text_bold">LAST NAME <span class="asterisk">*</span></div>
						</td>
						<td><input type="text" class="text" name="last_name" id="last_name" size="40" value="<?php echo $last_name; ?>" required></td>
					</tr>
					<tr>
						<td>
							<div class="text_bold">EMAIL <span class="asterisk">*</span></div>
						</td>
						<td><input type="text" class="text" name="email" id="email" size="40" value="<?php echo $email; ?>" required></td>
					</tr>
					<tr>
						<td>
							<div class="text_bold">PHONE <span class="asterisk">*</span></div>
						</td>
						<td><input type="text" class="text" name="phone" id="phone" size="40" value="<?php echo $phone; ?>" required></td>
					</tr>
					<tr>
						<td>
							<div class="text_bold">YOUR AFFILIATION <span class="asterisk">*</span></div>
						</td>
						<td>
							<div class="text">
								<input type="radio" name="affiliation" value="Student Unions" <?php if ($affiliation == "Student Unions") {
																									echo "checked";
																								}; ?> required>&nbsp;Student Unions&nbsp;&nbsp;&nbsp;
								<input type="radio" name="affiliation" value="Bookstore" <?php if ($affiliation == "Bookstore") {
																								echo "checked";
																							}; ?>>&nbsp;Bookstore&nbsp;&nbsp;&nbsp;
								<input type="radio" name="affiliation" value="Parking and Transportation" <?php if ($affiliation == "Parking and Transportation") {
																												echo "checked";
																											}; ?>>&nbsp;Parking and Transportation&nbsp;&nbsp;&nbsp;
								<input type="radio" name="affiliation" value="Other" <?php if ($affiliation == "Other") {
																							echo "checked";
																						}; ?>>&nbsp;Other&nbsp;&nbsp;&nbsp;
							</div>
						</td>
					...
					<tr>
						<td>
							<div class="text_bold">YOUR SUPERVISOR <span class="asterisk">*</span></div>
						</td>
						<td>
							<div class="text">
					...
					?>
						...
						?>
								<div class="text">
									...
									<span class="choose_shift_date"><?= $row['shift_date'] ?>&nbsp;<?= $dayOfWeek ?></span>
									(<?= $row['time_from'] ?> - <?= $row['time_to'] ?>)
									at <span class="location"><?= $row['location'] ?></span> (<?= $row['event'] ?>) -
									<span class="num_positions"><?= $row['num_positions'] ?></span> Available, <span class="num_positions" style="color:blue"><?= $employeesApplied ?></span> Signed up
	...
	<script>
		...
		switch (day) {
			case 1:
				dayInfo.innerHTML = "Showing Sundays";
				break;
			case 2:
				dayInfo.innerHTML = "Showing Mondays";
				break;
			case 3:
				dayInfo.innerHTML = "Showing Tuesdays";
				break;
			case 4:
				dayInfo.innerHTML = "Showing Wednesdays";
				break;
			case 5:
				dayInfo.innerHTML = "Showing Thursdays";
				break;
			case 6:
				dayInfo.innerHTML = "Showing Fridays";
				break;
			case 7:
				dayInfo.innerHTML = "Showing Saturdays";
				break;
			default:
				dayInfo.innerHTML = "Showing All days";
		}

		document.getElementById("dropbtn").addEventListener("click", function(event) {
			event.preventDefault();
			var val = document.getElementById("daySelectionDropdown").value
			if (!val) {
				val = 8;
			}

			window.location.href = "./index.php?day=" + val;
		})
	</script>
</body>
<?php page_finish(); ?>