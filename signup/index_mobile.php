...
<table class="table" width="100%" border="0" cellspacing="0" cellpadding="5">
  <tbody>
    <tr>
      ...
      <td><input type="text" class="text form-control" name="first_name" id="first_name" size="40" value="<?php echo $first_name; ?>" required></td>
    </tr>
    <tr>
	  <td><div class="text_bold ">LAST NAME <span class="asterisk">*</span></div></td>
      <td><input type="text" class="text form-control" name="last_name" id="last_name" size="40" value="<?php echo $last_name; ?>" required></td>
    </tr>
    <tr>
      <td><div class="text_bold ">EMAIL <span class="asterisk">*</span></div></td>
      <td><input type="text" class="text form-control" name="email" id="email" size="40" value="<?php echo $email; ?>" required></td>
    </tr>
    <tr>
      <td><div class="text_bold ">PHONE <span class="asterisk">*</span></div></td>
      <td><input type="text" class="text form-control" name="phone" id="phone" size="40" value="<?php echo $phone; ?>" required></td>
    </tr>
    <tr>
      <td><div class="text_bold ">YOUR AFFILIATION <span class="asterisk">*</span></div></td>
      <td>
			<div class="text radios">
					<input type="radio" id="radio1" name="affiliation" value="Student Unions" <?php if ($affiliation == "Student Unions") { echo "checked"; }; ?> required />
					<label for="radio1">Student Unions</label>
<br>
					<input type="radio" id="radio2" name="affiliation" value="Bookstore" <?php if ($affiliation == "Bookstore") { echo "checked"; }; ?> />
					<label for="radio2">Bookstore</label>
<br>
					<input type="radio" id="radio3" name="affiliation" value="Parking and Transportation" <?php if ($affiliation == "Parking and Transportation") { echo "checked"; }; ?> />
					<label for="radio3">Parking and Transportation</label>
<br>
					<input type="radio" id="radio4" name="affiliation" value="Other" <?php if ($affiliation == "Other") { echo "checked"; }; ?> />
...
$query = "SELECT S.*, location FROM Shifts S LEFT JOIN Locations L ON S.location_id = L.id  WHERE S.id NOT IN (SELECT shift_id FROM Signups WHERE employee_id = " . $employee_id . ") ORDER BY shift_date ASC";
		
// Display already applied shifts.
$query_signup = "SELECT S.*, location, employee_id, status FROM Shifts S LEFT JOIN Locations L ON S.location_id = L.id RIGHT JOIN Signups SU ON S.id = SU.shift_id WHERE employee_id = " . $employee_id . " ORDER BY shift_date ASC;";	
?> 
<div class="choose_shift">
<div class="major_heading">Sign Up</div>
<?php
