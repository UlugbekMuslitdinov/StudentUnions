...
<h5 class="mb-0 menu-section-title" style="color: #f58220; font-size: 19px; border-bottom: 2px solid #c8942b;">Beverages</h5>
<table class="small-12 large-12 columns" style="width: 100%"><tbody>
    ...
    @foreach (['neighborhood_blend', 'neighborhood_decaf', 'vanilla_hazelnut', 'seasonal_roast', 'orange_juice', 'tea_forTheGroup', 'lemonade_forTheGroup', 'blackberryLemonade_forTheGroup', 'hot_tea', 'hon_tea_green', 'hon_tea_half', 'vitamin_acai', 'vitamin_tropical', 'vitamin_strawberry', 'coke', 'diet_coke', 'dr_pepper', 'sprite', 'fanta_orange', 'dasani', 'gallon_water'] as $key)

        {{--  --}}
        @if ($data->beverage->list[$key]['qty'] != 0)
        <tr class="first-item" style="font-size: 15px;">
            <td class="first-item" style="font-size: 14px;">{{$data->beverage->list[$key]['name']}}</td><td class="text-center" style="font-size: 14px; text-align: left;">${{$data->beverage->list[$key]['price']}}</td><td class="text-center" style="font-size: 14px; text-align: right;">{{$data->beverage->list[$key]['qty']}}</td><td class="text-right" style="font-size: 14px; text-align: right;">${{round($data->beverage->list[$key]['total'],2)}}</td>
        </tr>
        @endif
        {{-- End :  --}}
    ...
    <tr class="od-footer" style="border-top: 1px solid #c8942b;">
        <td colspan="3" style="border-top: 1px solid #c8942b;"></td><td class="text-right" style="font-size: 14px; border-top: 1px solid #c8942b; text-align: right;">${{$data->beverage->total}}</td>
    </tr>
