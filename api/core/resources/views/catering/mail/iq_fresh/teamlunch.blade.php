@if (isset($data->teamlunch) && $data->teamlunch->total != 0)
<h5 class="mb-0 menu-section-title" style="color: #f58220; font-size: 19px; border-bottom: 2px solid #c8942b;">Smart Lunch for the Team</h5>
<table  class="small-12 large-12 columns" style="width: 100%"><tbody>
    ...
    @if (isset($data->teamlunch->total) && $data->teamlunch->total != 0)

            <tr class="first-item" style="font-size: 15px;">
                <td>Smart Lunch for the Team</td><td></td><td></td><td class="text-right"></td>
            </tr>

            @if ($data->teamlunch->qty != 0)

            <tr class="child-item"  style="font-size: 14px;">
            <td style="padding-left: 10px; font-size: 14px;"> - Dozen</td><td class="text-center" style="font-size: 14px; text-align: right;">${{$data->teamlunch->price}}</td><td class="text-center" style="font-size: 14px; text-align: right;">{{$data->teamlunch->qty}}</td><td class="text-right" style="font-size: 14px; text-align: right;">${{$data->teamlunch->qty * $data->teamlunch->price}}</td>
	    </tr>
	    <tr class="child-item"  style="font-size: 14px;">
            <td style="padding-left: 10px; font-size: 14px;"> - Half Dozen</td><td class="text-center" style="font-size: 14px; text-align: right;">${{$data->teamlunch->price_half}}</td><td class="text-center" style="font-size: 14px; text-align: right;">{{$data->teamlunch->qty_half}}</td><td class="text-right" style="font-size: 14px; text-align: right;">${{$data->teamlunch->qty_half * $data->teamlunch->price_half}}</td>
	    </tr>

            <tr class="sub-item" style="margin-top: 5px;">
                <td colspan="4" style="padding-left: 30px;">
                    @foreach ($data->teamlunch->list as $item)
                        @if (count($item) != 0)
                            <p style="font-size: 12px; border: 1px solid #ececec; padding: 5px;">
                                ...
                                @foreach ($item['choice_one']['list'] as $key => $value)
                                    ...
                                    @if ($value != 0)
                                        {{ $data->teamlunch->names[$key]['name'] }} : {{ $value }}<br />
                                    @endif
                                @endforeach
    ...
    <tr class="od-footer" style="border-top: 1px solid #c8942b;"><td colspan="3" style="border-top: 1px solid #c8942b;"></td><td class="text-right" style="border-top: 1px solid #c8942b; text-align: right;">${{$data->teamlunch->total}}</td></tr>
</tbody></table>
