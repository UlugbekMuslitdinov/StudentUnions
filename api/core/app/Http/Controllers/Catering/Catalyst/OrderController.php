<?php
namespace App\Http\Controllers\Catering\Catalyst;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use App\Mail\CatalystEmail;
...
class OrderController extends Controller 
{
    ...
    public function submitOrder(Request $request){
        ...
        if (session('order_id') == null){
            ...
            $objData->to = 'customer';
            $result = Mail::to($customer_info->customer_email)->send(new CatalystEmail($objData));

            $objData->to = 'manager';
            // $result = Mail::to('su-retailcatering@email.arizona.edu')->send(new CatalystEmail($objData));
            $result = Mail::to('su-web@email.arizona.edu')->send(new CatalystEmail($objData));
