...
<title>Preview Menu ~ Mesa Room Menu System</title>
...
<h3 style="margin-bottom:0; margin-top:15px;">First Course</h3>
<?php 
...
if (!$display) {
	echo '<p><span style="color:#ff0000;">No first course to display.</span><br />
	The "First Course" header and this message will be hidden on the live menu.</p>';
}
?>

<h3 style="margin-bottom:0; margin-top:15px;">Entr&eacute;es</h3>
<?php 
...
while (($itemRow = $itemResult->fetch_assoc()) || ($count <= $maxEntrees)) {
	...
	if ($itemRow['name'] != '') {
		echo $itemRow['name'] . '<br />';
		echo '<ul class="previewBulls"><li>' . $itemRow['description'] . '</li></ul>';
		$display = true;
	}
	$count++;
}
...
<h3 style="margin-bottom:0;">Dessert</h3>
<?php 
...
if (!$display) {
	echo '<p><span style="color:#ff0000;">No desserts to display.</span><br />
	The "Dessert" header and this message will be hidden on the live menu.</p>';
}
?>
