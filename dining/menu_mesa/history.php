...
<title>Menu Admin Update Log ~ Mesa Room Menu System</title>
...
?>
	...
	<div style="padding:10px 20px;">
		...
		while ($eventRow = $event->fetch_assoc()) {
			...
			<td class="history">' . $eventRow['menu'] . '</td>';
			

			// query to get all first courses updated during a specific event
			$eventFirstCourse = $db->query("SELECT * FROM history_event_content WHERE id = '" . $eventRow['id'] . "' AND dishID = " . $firstCourses . " ");
			
			echo '<td class="history"><ol class="historyNums">';
			while ($eventFirstCourseRow = $eventFirstCourse->fetch_assoc()) {
				echo '<li>' . $eventFirstCourseRow['name'] . '</li>';
			}
			echo '</ul></td>';
			...
			while ($eventEntreeRow = $eventEntree->fetch_assoc()) {
				echo '<li>' . $eventEntreeRow['name'] . '</li>';
					echo '<ul><li>' . $eventEntreeRow['description'] . '</ul></li>';
			}
			echo '</ul></td>';
			
			
			// query to get all desserts updated during a specific event
			$eventDessert = $db->query("SELECT * FROM history_event_content WHERE id = '" . $eventRow['id'] . "' AND dishID = " . $desserts . " ");

			echo '<td class="history"><ol class="historyNums">';			
			while ($eventDessertRow = $eventDessert->fetch_assoc()) {
				echo '<li>' . $eventDessertRow['name'] . '</li>';
			}
			echo '</ul></td>';
