...
<title>Edit Menu ~ Cactus Grill Menu System</title>
...
<h2>Edit <em style="color:green;">Hot Line Dinner</em> Menu</h2>
<?php
if (!$_POST) {
	echo '<p style="padding-left:10px; color:#ff0000; font-size:9px;">Please note, "Grains" and "Vegetarian" dish types are intentionally disabled. Only "Entr&eacute;es" and "Sides" apply to the Cactus Grill Hot Line Dinner menu.</p>';
}
?>
...
while (($itemRow = $itemResult->fetch_assoc()) || ($count <= $maxGrains)) {
	echo $count . '. <input type="text" disabled="disabled" maxlength="100" size="30" name="grain[' . $count . ']" value="' . $itemRow['name'] . '" /> $<input type="text" disabled="disabled" maxlength="5" size="3" name="priceGrains[' . $count . ']" value="' . $itemRow['price'] . '" /><br />';
	$count++;
}
echo '&nbsp;<br />';
?>
...
while (($itemRow = $itemResult->fetch_assoc()) || ($count <= $maxSalads)) {
	echo $count . '. <input type="text" disabled="disabled" maxlength="100" size="30" name="salad[' . $count . ']" value="' . $itemRow['name'] . '" /> $<input type="text" disabled="disabled" maxlength="5" size="3" name="priceSalads[' . $count . ']" value="' . $itemRow['price'] . '" /><br />';
	$count++;
}
echo '&nbsp;<br />';
?>
...
<?php
	...
	// 	// force price to two decimals if a price is present
	// 	if ($priceGrains[$tmp]) {
	// 		$priceGrains[$tmp] = number_format($priceGrains[$tmp],$num_dec_places);
	// 	}
	...
	// 	// force price to two decimals if a price is present
	// 	if ($priceSalads[$tmp]) {
	// 		$priceSalads[$tmp] = number_format($priceSalads[$tmp],$num_dec_places);
	// 	}
	...
	while ($tmp <= $maxEntrees) {
		...
		// force price to two decimals if a price is present
		if ($priceEntrees[$tmp]) {
			$priceEntrees[$tmp] = number_format($priceEntrees[$tmp],$num_dec_places);
		}
	...
	while ($tmp <= $maxSides) {
		...
		// force price to two decimals if a price is present
		if ($priceSides[$tmp]) {
			$priceSides[$tmp] = number_format($priceSides[$tmp],$num_dec_places);
		}
