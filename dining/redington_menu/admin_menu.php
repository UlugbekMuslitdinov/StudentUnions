...
<title>Redington Menu System</title>
...
//	echo '<p style="padding-top:20px; text-align:center;"><a href="https://webauth.arizona.edu/webauth/logout?logout_href=http://' . $_SERVER['SERVER_NAME'] . '/dining/redington_menu/&logout_text=Return%20to%20Redington Menu">Logout of UA NetID WebAuth</a></p>';
	echo '</div>';
	session_unset();
} else {
	...
	if ($_POST['wk1Status']) {
		...
		$update = "UPDATE week SET 
		status = \"" . $_POST['wk1Status'] . "\"
		WHERE id = 1";

		if (!mysql_query($update,$DBlink)) {
			echo '<br /><p>There was a problem saving your comments to our database. Please try again. If you continue to experience problems, please email <a href="mailto:kmbeyer@email.arizona.edu"> kmbeyer@email.arizona.edu</a>.</p>';
			// don't show the errors on the live application
			//die('Error: ' . mysql_error());
			die();
		} else {
			// confirmation message
			//echo '<p>You have successfully updated the board member. Please <a href="javascript:parent.close()">close</a> this window to continue.</p>';
		} // END IF for query
		
	} else if ($_POST['wk2Status']) {
...
<tr>
	<td colspan="6" style="text-align:left;"><p style="padding-bottom:3px;"><strong>Welcome to the Redington Menu Management System</strong></p>
	...
	<p style="text-align:right;"><a href="/dining/sumc/redington" target="_blank">View Live Menu</a> | <a href="history.php">View Admin Log</a></p></td>
</tr>
<tr bgcolor="#CCCCCC">
	<td style="text-align:center; padding:5px 10px 5px 10px;"><a href="admin_menu.php" title="Refresh: click here if you've changed a menu name but are not seeing the updated name in the calendar grid below"><img src="/dining/redington_menu/refresh.gif" width="25" height="25" align="bottom" /></a></td>
...
$start = mysql_query("SELECT * FROM start");
$startRow = mysql_fetch_array($start);
...
$weekCount = $weeksPast % 3;
	...
	case 0:
		...
		$week = mysql_query("SELECT * FROM week ORDER BY id ASC");

		// arrange the weeks' closed/open status fields accordingly
		// get results for 1st week
		$weekRow = mysql_fetch_array($week);
		// put week1 in week1
...
<!-- BEGIN DAY 1 -->
	...
	<?php	
		...
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day1 . "");
		$dayRow = mysql_fetch_array($day);
		
		// query the menu table to figure out the name of the menu based on the menu ID for this day
		$menuName = mysql_query("SELECT * FROM menu WHERE id = " . $dayRow['menuID'] . "");
		$menuNameRow = mysql_fetch_array($menuName);
		...
		$menu = mysql_query("SELECT * FROM menu ORDER BY name");
	...
	<?php
		if ($menuNameRow['name'] == 'Closed') {
			...
			echo '<br /><a href="javascript:;" onclick="javascript:window.open(\'admin_menu_edit.php?menu=' . $dayRow['menuID'] . '\',\'View\',\'width=725, height=750, scrollbars=yes, resizable=yes, toolbar=yes, location=yes\')"><img src="/dining/redington_menu/edit.gif" width="20" height="22" alt="Edit" title="Edit" /></a>';
			echo '&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:;" onclick="javascript:window.open(\'admin_menu_preview.php?menu=' . $dayRow['menuID'] . '\',\'View\',\'width=725, height=750, scrollbars=yes, resizable=yes, toolbar=yes, location=yes\')"><img src="/dining/redington_menu/preview.gif" width="20" height="22" alt="Preview" title="Preview" /></a>';
...
<!-- BEGIN DAY 2 -->
	...
	<?php
		...
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day2 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 3 -->
	...
	<?php
		...
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day3 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 4 -->
	...
	<?php
		...
		// query the day table for day 
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day4 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 5 -->
	...
	<?php
		...
		// query the day table for day 
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day5 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 6 -->
	...
	<?php
		...
		// query the day table for day 
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day6 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 7 -->
	...
	<?php
		...
		// query the day table for day 
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day7 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 8 -->
	...
	<?php
		...
		// query the day table for day 
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day8 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 9 -->
	...
	<?php
		...
		// query the day table for day 
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day9 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 10 -->
	...
	<?php
		...
		// query the day table for day 
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day10 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 11 -->
	...
	<?php
		...
		// query the day table for day 
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day11 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 12 -->
	...
	<?php
		...
		// query the day table for day 
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day12 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 13 -->
	...
	<?php
		...
		// query the day table for day 
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day13 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 14 -->
	...
	<?php
		...
		// query the day table for day 
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day14 . "");
		$dayRow = mysql_fetch_array($day);
...
<!-- BEGIN DAY 15 -->
	...
	<?php
		...
		// query the day table for day 
		$day = mysql_query("SELECT * FROM day WHERE id = " . $day15 . "");
		$dayRow = mysql_fetch_array($day);
