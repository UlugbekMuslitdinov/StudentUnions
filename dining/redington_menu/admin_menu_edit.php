...
function cleanFormData($formData) {
	$formData = mysql_real_escape_string($formData); // clean for mysql entry
...
<title>Edit Menu ~ Redington Restaurant Menu System</title>
...
Yes <input type="radio" value="Yes" name="display" <? if ($nameRow['display'] == "Yes") {echo 'checked="checked"';} ?> />&nbsp;&nbsp;&nbsp;&nbsp;No <input type="radio" value="No" name="display" <? if ($nameRow['display'] == "No") {echo 'checked="checked"';} ?> /><br />&nbsp;<br />
...
while (($itemRow = mysql_fetch_array($itemResult)) || ($count <= $maxSoups)) {
	echo $count . '. <input type="text" maxlength="100" size="30" name="soup[' . $count . ']" value="' . $itemRow['name'] . '" /><br />';
	$count++;
}
echo '&nbsp;<br />';
?>
...
while (($itemRow = mysql_fetch_array($itemResult)) || ($count <= $maxSalads)) {
	echo $count . '. <input type="text" maxlength="100" size="30" name="salad[' . $count . ']" value="' . $itemRow['name'] . '" /><br />';
	$count++;
}
echo '&nbsp;<br />';
?>
...
while ($commentRow = mysql_fetch_array($commentResult)) {
	echo '<p style="padding-left:70px; font-size:10px; color:#999999;">500 characters max</p>';
	echo '<textarea style="width:227px" rows="7" name="comments" onKeyDown="textCounter(this.form.comments,this.form.remLen,500);" onKeyUp="textCounter(this.form.comments,this.form.remLen,500);">' . $commentRow['comments'] . '</textarea><br />';
	echo '<input readonly type=text name=remLen size=3 maxlength=3 value="500" style="padding-left:2px; font-size:10px; color:#999999;"> <span style="padding-left:2px; font-size:10px; color:#999999;">characters remaining</span>';
}
...
while (($itemRow = mysql_fetch_array($itemResult)) || ($count <= $maxSides)) {
	echo $count . '. <input type="text" maxlength="100" size="30" name="side[' . $count . ']" value="' . $itemRow['name'] . '" /><br />';
	$count++;
}
echo '&nbsp;<br />';
?>
...
<?php
	...
	if ($menu != '') {
		...
		echo '<br /><p>There was a problem saving the menu changes to the database. Please email <a href="mailto:kmbeyer@email.arizona.edu">kmbeyer@email.arizona.edu</a> for assistance.</p>';
	...
	$result = mysql_query("DELETE FROM item WHERE menuID = " . $_GET['id'] . " AND dishID = " . $soups . "");

	// assign some tmp values for short term calculations
	$tmp = 1;
	$tmp1 = 1;

	// loop through all of the potential soup values from the form
	while ($tmp <= $maxSoups) {
		
		// assign the soup values from the form
		$soup[$tmp] = cleanFormData($_POST['soup'][$tmp1]);
		...
		if ($soup[$tmp] != '') {
			...
			$queryEventContent = "INSERT INTO history_event_content
			(id, name, dishID)
			VALUES ('$keyID', '$soup[$tmp]', $soups)";
		
			// check for errors saving to the db
			if (!mysql_query($queryEventContent,$DBlink)) {
	...
	$result = mysql_query("DELETE FROM item WHERE menuID = " . $_GET['id'] . " AND dishID = " . $salads . "");
	...
	$result = mysql_query("DELETE FROM item WHERE menuID = " . $_GET['id'] . " AND dishID = " . $entrees . "");
	...
	while ($tmp <= $maxEntrees) {
		...
		if ($entree[$tmp] != '') {
			...
			$queryEventContent = "INSERT INTO history_event_content
			(id, name, dishID)
			VALUES ('$keyID', '$entree[$tmp]', $entrees)";
	...
	$result = mysql_query("DELETE FROM item WHERE menuID = " . $_GET['id'] . " AND dishID = " . $sides . "");
	...
	#############################################
			...
			// set up the query to save the history of this update
			$queryEvent = "INSERT INTO history_event
			(id, netID, menu, display, timestamp)
			VALUES ('$keyID', '$netID', '$menu', '$display', $timestamp)";
		
			// check for errors saving to the db
			if (!mysql_query($queryEvent,$DBlink)) {
