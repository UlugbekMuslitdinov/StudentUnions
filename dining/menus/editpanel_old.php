			...
			<div id="contentlist">
				<?php
					...
					while($category_row = mysql_fetch_assoc($current)) {
						...
						echo '<hr>';
						// If the catagory or the panel are locked then everything below is locked
						if($category_row['locked'] || $panel_locked) {
							echo '<div style="float:left;width:100%;">';
							/********************* If the category is locked then display the locked category and its items *********************/
							echo('<table vAlign="middle" height="27px" cellspacing="0" cellpadding="0"><tr><td>'.$category_row['name'].'&nbsp&nbsp&nbsp</td>');
							...
							echo('<div style="float:left;width:100%;margin-left:20px;">');
						...
						/********************************************* The Category is not locked *********************************************/
							...
							// Layout the div which will contain the items in this category
							echo('<div style="float:left;width:610px;margin-left:20px;">');
							$cur_item = mysql_query("select * from item where categoryID='".$category_id."'");
							while($item_row = mysql_fetch_assoc($cur_item)) {
								echo '<div style="float:left;width:100%;">';
							...
							// Places the horizontal rule under an editable item/category
							echo('</div>');
						}
					}
				?>
