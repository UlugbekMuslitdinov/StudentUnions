<?php session_start();
	...
	if ($_POST['submit_type']=='cat') {
		...
		if (isset($_POST['cat_locked'][0])) {
			mysql_query('UPDATE category SET locked=true where id='.$_POST['cat_id']);
		}
		else {
			mysql_query('UPDATE category SET locked=false where id='.$_POST['cat_id']);
		}
	...
	if ($_POST['submit_type']=='item') {
		...
		if (isset($_POST['item_locked'][0])) {
			mysql_query('UPDATE item SET locked=true where id='.$_POST['item_id']);
		}
		else {
			mysql_query('UPDATE item SET locked=false where id='.$_POST['item_id']);
		}
	...
	header('Location: ./adminpanel.php?unitID='.$unit_id.'&panelID='.$panel_id);
	exit;
