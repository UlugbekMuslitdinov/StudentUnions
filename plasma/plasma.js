...
setInterval(switch_images, 20000);
...
// this function sets up an interval where 
// change_op() gets called every 25 
// milliseconds, until the interval is 
// reset within the called function.
...
function change_op()
{
  ...
  if(current=='image_1')
  {
    ...
    // if the current image is at or below zero
    // opacity, that is, if it is invisible, we
    // want to stop the interval and do some 
    // housekeeping.
    if(image_1.style.opacity <= 0)
    {
      // the clearInterval function
      // stop the repetitive calling
      // of this function.
      clearInterval(inter);
      ...
      // we call the ajax function to
      // update the invisible image
      // by opening another page.
      updateBlock();
    }
    else
    {
      // if the image is still visible, we
      // decrease its opacity by 5%.
      image_1.style.opacity -= .05;
      
      // we increase the opacity of the other image by the same amount,
      // by subtracting the opacity of the first image from 1 (100%).
      image_2.style.opacity = 1-(image_1.style.opacity);
    }
  }
  else if(current=='image_2') 
  {
    ...
    else
    {
      // if the image is still visible, we
      // decrease its opacity by 5%.
      image_2.style.opacity -= .05;
      
      // we increase the opacity of the other image by the same amount,
      // by subtracting the opacity of the first image from 1 (100%).
      image_1.style.opacity = 1-(image_2.style.opacity);
    }
  }
}
function firstLoad(){
